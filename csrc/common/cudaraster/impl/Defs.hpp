...
//
// RDNA (wave32): mask params cast to 64-bit as required by HIP.
// CDNA (wave64): half-wavefront emulation — blockDim.x is always 32,
// so two consecutive threadIdx.y rows share one wave64.  We extract
// the correct 32-bit half from the 64-bit ballot/match result using
// __lane_id().  Functions are defined BEFORE the macros so their bodies
// call the real ROCm builtins.
